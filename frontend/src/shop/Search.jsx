...
const Search = () => {
  ...
  const [pager, setPager] = useState(search);
  ...
  */
  ...
  useEffect(
    () => {
      setIsLoading(true);
      console.log("The searchword is " + search);
      var curr_products = {};
      var key_str = "cache_" + search + "_" + active + "_" + minPrice + "_";
      ...
      if (cachedProducts[key_str]) {
        ...
        Axios.post(
          baseURL + `/storefront/products/search/${search}/${active}`,
          {
            minPrice: minPrice,
            maxPrice: maxPrice,
          }
  ...
  if (!isLoading) {
    if (products.length) {
      return (
        <>
          <h1 className="cat-name">Search Results for "{search}"</h1>
